...
resource "ansible_group" "ecs-workers" {
  name = "ecs_workers"
  variables = {
...
resource "ansible_group" "ecs-masters" {
  name = "ecs_masters"
  variables = {
...
resource "ansible_group" "ecs-nodes" {
  name = "ecs_nodes"
  children = [
    ansible_group.ecs-masters.name,
    ansible_group.ecs-workers.name
  ]
}
...
resource "ansible_host" "ecs-masters" {
  for_each = { for idx, host in module.ecs-masters.hosts : idx => host }
...
resource "ansible_host" "ecs-workers" {
  for_each = { for idx, host in module.ecs-workers.hosts : idx => host }
